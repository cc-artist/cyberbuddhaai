...
const CommentScroll: React.FC = () => {
  ...
  const commentWidth = 180; // 调整宽度以适应5个评论
  ...
  const loadComments = async () => {
    ...
    try {
      ...
      setLoadError('Network error, using cached comments');
    }
    ...
    if (storedComments) {
      try {
        ...
        setComments([]);
        setLoadError('No comments available');
      }
    } else {
  ...
  const translateX = -currentGroupIndex * commentsPerGroup * commentWidth;
          ...
          >
            <div 
              ...
              style={{
                ...
                width: `${Math.ceil(displayComments.length / commentsPerGroup) * commentsPerGroup * commentWidth + (Math.ceil(displayComments.length / commentsPerGroup) - 1) * 12}px`,
              }}
            >
